...
class Pet(Base):
    ...
    # methods
    def __repr__(self):
        return "{}: {}".format(self.name, self.age, self.adopted)
...
class Shelter(Base):
    ...
    # methods
    def __repr__(self):
        return "{}: {}".format(self.name, self.website_address)
...
if __name__ == "__main__":
    ...
    log.info("Creating Shelters: NYC and NOLA (Transient)")
    nyc = Shelter(name="NYC Pet Orphanage", website_address="http://www.nyc-pet-orphanage.com")
    nola = Shelter(name="New Orleans Pet Hotel",website_address="http://www.neworleanspethotel.com")

    db_session.add_all( [nyc, nola] ) 
    db_session.commit()

    assert cat.id != None
    assert dog.id != None
    log.info("Shelters have been created nyc.id is {} and nola.id is {}".format(type(nyc.id), type(nola.id) ))
